...
import { profit, getFinancialAnalysisLevel,getFinancialAnalysisTime } from '@/api/finance';
/**
 * 数组转树形结构
 * @param list 源数组
 * @param tree 树
...
　　　　},
        ...
        echartData:null,
        loading:false,
        headerOne:{title:'年度',children:[{title:'年-季',children:[{title:'利润表',dataIndex:'name',key:'name'}]}]},
        checked:false,
    ...
    methods: {
      ...
      loadData() {
        ...
        // 财务分析分类
        getFinancialAnalysisLevel({type:2}).then((res) => {
          if(res.code ==1){
        ...
        // 财务分析时间
        getFinancialAnalysisTime({type:2}).then((res) => {
          if(res.code ==1){
