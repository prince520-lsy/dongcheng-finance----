...
<template>
  <div>
    ...
    <div class="flex justify-between mid-bg" :style="{'height':minHeight+'px'}">
      ...
      <div class="flex-grow-1" v-if="echartData">
        ...
        <a-table v-if="tableData" :columns="columns" :data-source="tableData" :pagination="false" :defaultExpandedRowKeys="defaultTableIds" rowKey='id'  :scroll="{  y: tableheight }"  :bordered="theme!='dark'"></a-table>
...
import { assetsLiability, getFinancialAnalysisLevel,getFinancialAnalysisTime } from '@/api/finance';
/**
 * 数组转树形结构
 * @param list 源数组
 * @param tree 树
...
　　　　},
        ...
        echartData:null,
        loading:false,
        headerOne:{title:'年度',children:[{title:'年-季',children:[{title:'资产负债',dataIndex:'name',key:'name'}]}]},
        checked:false,
    ...
    methods: {
      ...
      assetsLiability(){
        ...
        assetsLiability(params).then((res) => {
          if(res.code ==1){
